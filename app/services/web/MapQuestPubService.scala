...
object MapQuestPubService {
  ...
  val sizes = Vector(0F, 2500F, 630F, 78F, 20F, 2.4F, 0.61F, 0.076F, 0.019F)

  def serviceURLFor(lat: Float, lng: Float, radius: Float, matches: Int) =
    s"""http://www.mapquestapi.com/search/v2/radius?key=Fmjtd%7Cluur210znl%2C8x%3Do5-90ysqa&maxMatches=$matches&origin=$lat,$lng&hostedData=mqap.ntpois|group_sic_code=?|581208&radius=$radius&units=k"""

  def pubsForLoc(lat: Float, lng: Float, size: Int, matches: Int): Future[PubSeq] = {
    require(size > 0 && size <= 8, "Size must be between 1 and 8")
